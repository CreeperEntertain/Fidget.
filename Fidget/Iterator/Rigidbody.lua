...
local function vround(v1)
  return vec3(math.round(v1.x), math.round(v1.y), math.round(v1.z))
end
...
local function getCollidingFaceNormals(rigidbody1, rigidbody2, minIndex, mtv, axis)
  ...
  if minIndex > 0 then
    ...
    faceNormal1 = mtv:normalized()
    faceNormal2 = -mtv:normalized()
    i2 = -minIndex
  end
...
local function createContactPointsForCollidingBodies(
  ...
  local reducedVerts = {}
  local n = 0
  if i1 then -- i2 does not exist when particles are colliding with cubes
  ...
  verts = clipPolygonAgainstPlane(verts, -n2, dot(lpos + theAxisInQuestion2R2, -n2))
    ...
    verts = clipPolygonAgainstPlane(
      clipPolygonAgainstPlane(
        clipPolygonAgainstPlane(clipPolygonAgainstPlane(verts, n1, dot(lpos - theAxisInQuestion1R2, n1)), -n1,
          dot(lpos + theAxisInQuestion1R2, -n1)), n2, dot(lpos - theAxisInQuestion2R2, n2)), -n2,
      dot(lpos + theAxisInQuestion2R2, -n2))
...
function events.world_tick()
  ...
  local relaxation = physicsSim
      .relaxation --This name is takes so lets say this is made in china knockoff of the technique
          ...
          if not (rigidbody1.isSleeping and rigidbody2.isSleeping) then
            ...
            if mtv then
              ...
              if normalSnappingThreshold > 0 then
                ...
                local similarity = dot(faceNormal2, globalSnappingAxis1)
                ...
                elseif similarity < -1 + normalSnappingThreshold then
                  ...
                  similarity = dot(faceNormal2, globalSnappingAxis2)
                  if similarity > 1 - normalSnappingThreshold then
                    faceNormal2 = globalSnappingAxis2
                  elseif similarity < -1 + normalSnappingThreshold then
                    faceNormal2 = -globalSnappingAxis2
                  else
                    similarity = dot(faceNormal2, globalSnappingAxis3)
                    if similarity > 1 - normalSnappingThreshold then
      ...
      for i, joint in lnext, joints do
        if (joint.rigidbody1.linearMovement or joint.rigidbody2.linearMovement) then
          ...
          elseif joint.type == "spring" then
            ...
            local springForce = (joint.stiffness * (-length(pos1 - pos2) + joint.distance) - dot((rigidbody2.vel + crossed(rigidbody2.rotVel, d1)) - (rigidbody1.vel + crossed(rigidbody1.rotVel, d2)), dir)) *
                dir * 0.9
...
particles:newParticle("end_rod", constraint.contactPoint1) particles:newParticle("end_rod", constraint.contactPoint2)
      ...
      if solver == "regularPGS" then
          ...
          for i, constraint in lnext, constraints do
            if not constraint.joint then
              if constraint.bodyTwo then
                ...
                local relativeVelocity = dot(constraint.normal,
                  (r1vel + crossed(r1rotVel, constraint.d1)) - (r2vel + crossed(r2rotVel, constraint.d2)))
                local lambda = constraint.invEffMass * (relativeVelocity) -
                    constraint.invEffMass *
                    (constraint.baumgarte and constraint.bias or 0) --impulse calc
                ...
                rigidbody1.rotVel = r1rotVel +
                    (rigidbody1.invInertiaTensorWORLD * constraint.d1CrossedWithNormal * impulse)
                rigidbody2.vel = r2vel - P * rigidbody2.invMass
                rigidbody2.rotVel = r2rotVel -
                    (rigidbody2.invInertiaTensorWORLD * constraint.d2CrossedWithNormal * impulse)
              else
        ...
        for i, constraint in lnext, constraints do
          ...
          elseif constraintType == "contactWorld" then
            ...
            cache.add(constraint.rigidbody1.index, tostring(constraint.rigidbody2), constraint.edges,
      ...
      elseif solver == "scuffedSplitImpulse" then
        ...
        for i, constraint in lnext, constraints do
          ...
          elseif constraintType == "contactWorld" then
            local rigidbody1 = constraint.rigidbody1
            cache.add(rigidbody1.index, tostring(constraint.rigidbody2), constraint.edges, constraint.normalIndex,
              constraint.impulse)
            local impulse = -constraint.invEffMass * constraint.bias
      ...
      for j, rigidbody in lnext, rigidbodies do
        ...
        if not rigidbody.isSleeping then
          ...
          if rigidbody.worldCollision then
            ...
            if rigidbody.type ~= "particle" then
              ...
              local block = (worldGetBlockState(rigidbody.pos)).properties
            end
          ...
          if rigidbody.type ~= "particle" then
            rotAcceleration = (rigidbody.torqueAccum * rigidbody.invInertiaTensorWORLD)
          ...
          rigidbody.vel = rigidbody.vel * rigidbody.damping ^ (1 / physicsIterations)
          if rigidbody.type ~= "particle" then
            rigidbody.rotVel = rigidbody.rotVel * rigidbody.rotationDamping ^ (1 / physicsIterations)
        ...
        if physicsSim.broadPhaseCollision == "aabb" or rigidbody.worldCollision then
          rigidbody.boundingAABB = calculateBoundingAABB(rigidbody.vertices, rigidbody.pos, rigidbody.type)
